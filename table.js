...
var squareThestores = new Thestores (11, 38, 1.9, "washingtonsq", "Washington Square");
...
function validateInput () {
  var validElement = document.getElementsByClassName("pickChoice");
  var pickedQuestion = 0;

  for (j = 0; j < validElement.length; j++){
    if (validElement[j] === ""){
      validElement[j].style.borderColor="rgba(35, 0, 189, 1)";
    }
    else {
      validElement[j].style.borderColor="white";
      pickedQuestion++;
    }
  }
  if (pickedQuestion === validElement.length) {
    var button = document.getElementById("submitButton");
    addStores();
  }
}

window.addEventListener("load",buildStorePage);
window.addEventListener("load",writeStoreData);
element.addEventListener('click', validateInput, false);
